...
final class McpBridgeAction extends ActionBase implements ContainerFactoryPluginInterface {
  ...
  public function defaultConfiguration(): array {
    return [
      ...
      'mcp_body_templates' => '',
      'mcp_max_iterations' => '1',
      'mcp_stop_on_text_match' => '',
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state): array {
    ...
    $form['mcp_body_templates'] = [
      '#type' => 'textarea',
      '#title' => $this->t('MCP body templates (JSON array)'),
      '#default_value' => $this->configuration['mcp_body_templates'],
      '#description' => $this->t('Optional JSON array of body templates to run in sequence. Each element supports tokens. Leave empty to use the single MCP body template.'),
    ];
    $form['mcp_max_iterations'] = [
      '#type' => 'textfield',
      '#title' => $this->t('MCP max iterations'),
      '#default_value' => $this->configuration['mcp_max_iterations'],
      '#description' => $this->t('Maximum number of MCP calls to execute in a loop.'),
    ];
    $form['mcp_stop_on_text_match'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Stop on response text match'),
      '#default_value' => $this->configuration['mcp_stop_on_text_match'],
      '#description' => $this->t('Optional substring that stops the MCP loop when found in the response text.'),
    ];
    $form['question_input'] = [
      '#type' => 'textarea',
  ...
  public function execute($entity = NULL, array $context = []): void {
    ...
    $mcp_body_templates = [];
    if (!empty($this->configuration['mcp_body_templates'])) {
      $templates = json_decode((string) $this->configuration['mcp_body_templates'], TRUE);
      if (is_array($templates)) {
        $mcp_body_templates = $templates;
      }
      else {
        $this->logger->warning('Invalid MCP body templates JSON provided; falling back to single template.');
      }
    }
    if (empty($mcp_body_templates)) {
      $mcp_body_templates = [(string) $this->configuration['mcp_body']];
    }

    $max_iterations = max(1, (int) $this->configuration['mcp_max_iterations']);
    $stop_text = (string) $this->configuration['mcp_stop_on_text_match'];
    $mcp_response_raw = '';
    $mcp_response_json = NULL;
    $mcp_response_text = '';

    for ($iteration = 0; $iteration < $max_iterations; $iteration++) {
      $template_index = min($iteration, count($mcp_body_templates) - 1);
      $mcp_body_template = (string) $mcp_body_templates[$template_index];
      ...
      if ($stop_text !== '' && str_contains($mcp_response_text, $stop_text)) {
        break;
      }
    }
